...
var vsh_ps3xploit_key_toc_488=0x707A1C;// Updated From 4.83/4.84/4.85
...
function dex()
{
	switch (fwVersion) {
		...
		case "4.87":
			if(document.getElementById('dex').checked===true){loadcex_487();}//alert("calling loaddex_484");
			else {loadcex_487();}
			disable_trigger();
			break;
			
		case "4.88":
			if(document.getElementById('dex').checked===true){loadcex_488();}//alert("calling loaddex_484");
			else {loadcex_488();}
			disable_trigger();
			break;
	...
	switch (uaStringCheck) {
		case "PLAYSTATION":
			switch (fwVersion) {
				...
				default:
					alert('Your PS3 is not on FW 4.84+! Your current running FW version is ' + fwVersion + ', which is not compatible with PS3Xploit Tools 3.0. All features have been disabled');
					disable_all();
					break;
			}
			break;
